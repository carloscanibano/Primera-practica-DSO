...
//Nombramos nuestra cola de procesos en espera
static struct queue *cola_espera_disco;
...
void disk_interrupt(int sig)
{
  if (!queue_empty(cola_espera_disco)) {
    TCB* listo;
    listo = dequeue(cola_espera_disco);
    listo->state = INIT;
    if (listo->priority == HIGH_PRIORITY) {
      ...
      enqueue(cola_baja_prioridad, listo);
      printf("*** THREAD %d READY\n", listo->tid);
    }
  }
}
...
void activator(TCB* next){
  ...
  //Si el thread actual es idle y el siguiente esta listo
